...
class Paginador
{
     ...
     */
    public function getHtmlPaginacion($varGet = 'pagina', $cont = 'li',$rutafinal)
    {
        $paginador = array();
        /*echo "<pre>";
                var_dump($this->_paginacion);
            echo "</pre>";*/
        ...
        foreach ($this->_paginacion as $enlace) {
            ...
            if ($enlace['numero'] != $this->_pagActual && empty ($enlace['off'])) {
                ...
                //$htmlPag   .= '<a href="' . $this->_urlDestino
                        //. $enlace['numero']
                        //. $this->_propagar . '" '
                        //. 'title="' . $enlace['title'] . '" '
                        //. '>' . $enlace['vista'] . '</a>';

                $htmlPag   .= '<a href="javascript:void(0)" onclick="Funciones.paginar(this);" data-url="' . $rutafinal.'?'.$varGet.'='
                        . $enlace['numero']
                        . $this->_propagar . '" '
                        . 'title="' . $enlace['title'] . '" '
                        . '>' . $enlace['vista'] . '</a>';
    ...
    {
        //echo "PRI_inicial ".$paginaInicial;
        if ($paginaInicial != 1 && !in_array('primero', $this->_omitir)) {
            //echo "PRI: A";
            $this->_paginacion[]    = array('numero'   => 1,
    ...
    {
        if($pagina > 1 && !in_array('anterior', $this->_omitir)) {
        ...
        for ( $f = $paginaInicial; $f <= $paginaFinal; $f++) {
            if ($f != $pagina && !in_array('numero', $this->_omitir)) {
                $this->_paginacion[]    = array('numero'    => $f,
                                                'vista'     => $this->_titulos['numero']['vista'] . ($f),
    ...
    {
        if ($pagina < ($totalPaginas) && !in_array('siguiente', $this->_omitir)) {
            $this->_paginacion[]    = array('numero'    => $pagina+1,
    ...
    {
        if ($paginaFinal <= ($totalPaginas - $this->_cantidadDeEnlacesDelPaginador) && !in_array('bloqueSiguiente', $this->_omitir)) {
            $this->_paginacion[]    = array('numero'    => $blockFinal,
    ...
    {
        if ( $paginaFinal != ($totalPaginas) && !in_array('ultimo', $this->_omitir)) {
            $this->_paginacion[]    = array('numero'    => $totalPaginas,
